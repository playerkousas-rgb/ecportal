...
>
> **最新修正（2026-09-20，v2.6.2）**：**「儲存唔到去後端／後端讀取唔到」搵到真正死因並修好** ——
...
>
> **v2.6.0（同日較早）**：**只有一個儲存方式** —— ① 登入嗰刻由後端攞資料（＝後端嗰刻嘅狀態）；
> ② 之後改乜都只存喺瀏覽器，直至你撳頂部「**儲存到後端（N）**」；③ 撳儲存先核對後端版本：有人喺你之後儲存過會話你知，
...
* **儲存只排隊**：改動永遠唔會自動寫後端，要撳頂部「儲存到後端（N）」（N ＝ 未寫入改動次數）
...
**只有一個方式**（2026-09-20 定案，詳見 [docs/UPDATES_2026-09-20.md](docs/UPDATES_2026-09-20.md)）：

1. **登入攞後端** —— 揀旅團 → 由後端（Google Sheet「資料庫」分頁）攞成份資料 → 讀帳戶 → 密碼登入。
   攞到嘅嗰份 ＝ 後端嗰一刻嘅狀態，會存做「基準」（`venture82.unit.<編號>.base.v2`）。
2. **改動只存喺瀏覽器** `localStorage`（`venture82.unit.<編號>.db.v2`），頂部顯示「儲存到後端（N）」。
   冇自動儲存、冇背景讀、冇閂視窗補寫。
3. **撳「儲存到後端」**：先核對後端版本；有人喺你登入後儲存過 → 話你知，並拉成份落嚟做**三方逐格比對**：
   * 唔同格（佢加張三、我加李四；佢點李小明、我點陳大文）→ 一齊儲存；同一格同一個值 → 冇問題；
   * **同一格唔同值**（佢登記「遲到」、我登記「早走」）→ 逐格列出、**未寫入**、要你剔咗「用我嘅」再確認先蓋。
* 未由後端載入過嘅裝置（冇基準）**唔准寫**；示範模式（`venture82.mock.db.v2`）永遠唔寫後端。
* 首次開啟（後端仲係空）會由 `data/units/<編號>/*.json` **種子載入**一次；第一次「儲存到後端」就建立後端正本。
* 備份：「帳號與系統 → 資料管理 → 匯出 JSON」；還原備份 ＝ 一次本機改動，照樣要撳「儲存到後端」，撞嘅格照樣會問。
...
| `node tests/smoke.mjs mock` | 示範模式：同樣項目 + 示範／真實隔離（**通告「同步到公開頁」一個請求都唔送後端**） | **518 / 0** |
