...
>
> **最新修正（2026-09-19，v2.5.1）**：① 過咗截止／完結嘅活動同通告**照顯示**（排後＋標「已截止／已完結」，
> 遲咗想報嘅會搵領袖／執委；報咗名嘅睇得返內容）。② **執委系統跨視窗同步**：同一帳戶無痕／普通視窗，
> 切返個視窗 1.2 秒即刻對版本；有未存改動都照對（自動合併）；舊版後端會彈警告。
...
> **自己睇進度＋申報完成**（等執委批核）。⑥ 多人一齊用：右上角長期有
> 「**立即儲存／立即同步**」掣、60 秒會議模式自動睇隊友更新、同一條紀錄**逐格合併**。
...
npm test             # build:gas + smoke(real/mock) + public + gate + api + progress + … + gas + hub（14 檔一條鏈）
```

`npm test` 全鏈 **1,795 項斷言 / 0 失敗（2026-09-19）**：

| 測試 | 內容 | 結果 |
|---|---|---|
| `node tests/hub.mjs`（**新**） | **團員入口端到端**：假 GAS 後端＋空白新裝置 → 開機拉後端 → YMIS＋1234 登入 → 強制改密碼 → RSVP（唔問名）→ 後端真係收到 → 過期活動／截止通告**照顯示（標已截止／已完結）** → **我的進度：申報→執委推 db→申報零損失→批核→團員見到已完成**（衝突模擬） → 登出零殘留 → 公開團章讀後端正本（冇名冊外洩） | **48 / 0** |
...
| `node tests/smoke.mjs real` | 真實模式：種子資料、登入／密碼權限、雙財政年度、AGM 逐年輸入、**團費收款紀錄（含領袖免收）**、生日、全部頁面渲染、**分頁對位**、QR、文件輸出、物資加減庫存、財務匯入解析、**帳目（本年度總覽／按月／過往紀錄・統計卡＝期初結餘→收入→支出→淨額→現在結餘）**、**報告唔混上年度結餘**、**通告（開一張・分享・報名・統計・撳真「開新通告」掣・同步到公開頁）**、**欄位設計（改名／還原，每頁入口）**、**插入自己嘅 Sheet**、**快速記帳**、**總表同步下載**、**進度直接接駁**、**新旅團申請經 `/api/proxy` 送去 ADMIN 系統（收件匣唔回執，送到就 OK）** | **544 / 0** |
| `node tests/smoke.mjs mock` | 示範模式：同樣項目 + 示範／真實隔離 | **523 / 0** |
...
| `node tests/tabs.mjs` | 分頁掣**真係撳得**（dispatch click 驗 hash）＋ **mock 分頁 super 限定** | **50 / 0** |
| `node tests/remote.mjs` · `login.mjs` · `api.mjs` · `progress.mjs` · `pdftext.mjs` · `bake.mjs` | 遠端同步（衝突／合併／版本、**跨視窗 focus 即拉**）、登入（密碼／暫用密碼）、API 轉發防護、進度讀寫、PDF 文字抽取、焗名單 | **147 · 24 · 32 · 33 · 47 · 15 / 0** |
