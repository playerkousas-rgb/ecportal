...
4. 進度系統：喺「進度 → 設定」填旅團自己嘅 VSBADGE `/exec` 網址同 API Key 就得
...
3. **進度系統**：而家係「直接接駁」——喺「進度 → 設定」填 VSBADGE `/exec` ＋ API Key。
   舊嘅入口連結（要對方登記本系統網址）仍然保留，但對方未登記時會出 `referer_mismatch`。
